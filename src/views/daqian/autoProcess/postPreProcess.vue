    ...
    <div class="postPreProcess">
            ...
            <div class="searchDiv">
                <div class="selectArea" :style="collapseFlag?'height:100%':'height:100px'">
                    ...
                    <div class="selectDiv">
                        ...
                        <selectProject class="cascader-select-style" @updateProjectStatus="updateProjectStatus" @selectProjects="SelectProjects" :ChangeOnselect="ChangeOnselect" :isProjectClearable="isProjectClearable" :havedefaultProject="havedefaultProject" :showSelectProject="showSelectProject" :resetselectProjectData="resetselectProjectData"></selectProject>
                    </div>
                    <div class="selectDiv">
                        <span class="align-right">项目类型：</span>
                        <el-select v-model="search.subProjectProcessType" placeholder="全部" clearable size="medium">
                            <template>
                                <el-option v-for="(item,index) in this.subProjectProcessAllTypes" :key="index" :value="index" :label="item"></el-option>
                            </template>
                        </el-select>
                    </div>
                    <div class="selectDiv">
                        ...
                        <el-select v-model="search.project_status" placeholder="全部" size="medium">
                            <template>
                                <el-option v-for="(item,index) in this.projectStatuDatas" :key="index" :value="index" :label="item"></el-option>
                            </template>
                        </el-select>
                    </div>
                    ...
                    <div class="selectDiv">
                        ...
                        <el-popover class="input-style" placement="bottom-start" title="支持双向模糊查询，所有查询支持空格、分号、逗号(中英文)分隔多个查询条件" trigger="focus" :content="search.sourceSubProjectName">
                            <el-input class="input-style" slot="reference" size="medium" v-model="search.sourceSubProjectName" clearable placeholder="搜索源子项目名称，支持模糊搜索">
                            </el-input>
                        </el-popover>
                    </div>
                    <div class="selectDiv">
                        ...
                        <el-popover class="input-style-1" placement="bottom-start" title="仅可输入数字、空格、分号、逗号，空格/分号/逗号分隔多个查询条件" trigger="focus" :content="search.tileID">
                            <el-input size="medium" slot="reference" v-model="search.tileID" :change="formatSearchInput()" clearable placeholder="搜索网格编号">
                            </el-input>
                        </el-popover>
                    </div>
                </div>
        ...
        <div class="table-area">
            ...
            <div class="btn-list">
                ...
                <el-button v-show="!(search.selectSection == 2001)" class="right-div" type="primary" size="medium" :disabled="accessDisabled" @click="operationTiles()">{{currentSectionOperations}}</el-button>
                ...
                <el-button v-show="search.selectSection == 2001 || search.selectSection == 2101" class="right-div" type="primary" size="medium" @click="downloadLogs()" :disabled="accessDisabled">log批量下载</el-button>
                <el-tooltip content="准出失败的可以尝试重新准出" placement="top-start" effect="light">
                    <el-button v-show="search.selectSection == 2001 || search.selectSection == 2101" class="right-div" type="primary" size="medium" @click="tryAllowOut()" :disabled="accessDisabled">重新准出</el-button>
                </el-tooltip>
            </div>
            <!-- 按钮区 end-->
            ...
            <div class="table-div">
                ...
                <div v-if="search.selectSection == 20 || search.selectSection == 24 || search.selectSection == 21 || search.selectSection == 23">
                    <el-table :empty-text="emptyText" v-loading="loading" :max-height="formHeight" element-loading-text="拼命查询中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(80, 80, 80, 0.8)" :data="tableData" ref="postPreProcessTables" border style="width: 100%" @select="tileSelectionChange" @select-all="tileSelectionChange" @sort-change="sortTable">
                ...
                <div v-else-if="search.selectSection == 22">
                    <el-table :empty-text="emptyText" v-loading="loading" element-loading-text="拼命查询中" :max-height="formHeight" element-loading-spinner="el-icon-loading" element-loading-background="rgba(80, 80, 80, 0.8)" :data="tableData" ref="postPreProcessTables" border style="width: 100%" @select="tileSelectionChange" @select-all="tileSelectionChange" @sort-change="sortTable">
                        <el-table-column fixed type="selection" width="55"></el-table-column>
                        <el-table-column fixed prop="tile_id" label="网格编号" show-overflow-tooltip sortable="custom" width="105"></el-table-column>
                        <el-table-column fixed prop="tile_level" label="网格层级" show-overflow-tooltip width="80"></el-table-column>
                        <el-table-column prop="show_sub_project_name" label="所属子项目" show-overflow-tooltip width="120"></el-table-column>
        ...
        <el-dialog title="后预处理操作" :visible.sync="dialogOperationTilesVisible" :close-on-click-modal="false" width="60%">
            <el-form label-width="120px" class="demo-dynamic" ref="operationForm" :model="operationData" :rules="operationFormRules">
                <el-form-item label="网格列表" prop="tileIDS">
                    <el-input v-model="operationData.tileIDS" readonly type="textarea" placeholder="请选择要操作的网格" :autosize="{ minRows: 5, maxRows: 5}"></el-input>
                </el-form-item>
                ...
                <el-form-item label="当前环节" required v-if="(search.selectSection == 22 || search.selectSection == 2101) && operationData.selectOperation==4" size="medium">
                    <el-select disabled v-model="operationData.currentTaskType" style="float: left" placeholder="选择环节" size="medium">
                        <el-option value="20" label="点云融合"></el-option>
                        <el-option value="22" label="点云融合人工调整与确认"></el-option>
                        <el-option value="23" label="P矩阵生成"></el-option>
                        <el-option value="21" label="点云识别+染色+车道线建模"></el-option>
                ...
                <el-form-item label="下一环节" required v-if="search.selectSection == 22 && operationData.selectOperation==4">
                    <el-select disabled v-model="operationData.NextTaskType" style="float: left" placeholder="选择环节" size="medium">
                        <el-option value="20" label="点云融合"></el-option>
                        <el-option value="22" label="点云融合人工调整与确认"></el-option>
                        <el-option value="23" label="P矩阵生成"></el-option>
                        <el-option value="21" label="点云识别+染色+车道线建模"></el-option>
        ...
        <el-dialog title="重新准出" :visible.sync="dialogTryAllowOutVisible" :close-on-click-modal="false">
            <el-form ref="tryAllowOutData" :model="tryAllowOutData" label-width="120px" class="demo-dynamic">
                <el-form-item label="网格列表" required prop="tileIDS" :rules="[{ required: true, message: '请选择要重新准出的网格', trigger: 'blur'},{ required: true, message: '请选择要重新准出的网格', trigger: 'change'}]">
                    <el-input type="textarea" readonly placeholder="请选择要重新准出的网格" :autosize="{ minRows: 5, maxRows: 5}" v-model="tryAllowOutData.tileIDS"></el-input>
                </el-form-item>
        ...
        <el-dialog title="准出log批量下载" :visible.sync="dialogDownloadLogVisible" :close-on-click-modal="false">
            <el-form ref="downloadLogData" :model="downloadLogData" label-width="120px" class="demo-dynamic">
                <el-form-item label="网格列表" size="medium" required prop="tileIDS" :rules="[{ required: true, message: '请选择要下载log的网格', trigger: 'blur'},{ required: true, message: '请选择要下载log的网格', trigger: 'change'}]">
                    <el-input type="textarea" readonly placeholder="请选择要下载log的网格" :autosize="{ minRows: 5, maxRows: 5}" v-model="downloadLogData.tileIDS"></el-input>
                </el-form-item>
        ...
        <el-dialog title="编辑网格后预处理信息" :visible.sync="dialogEditVisible" :close-on-click-modal="false">
            <el-form ref="inPostPreProcess" :model="inPostPreProcess" label-width="120px" class="demo-dynamic">
                ...
                <el-form-item label="触发时间">
                    <el-col :span="18">
                        <el-date-picker format type="datetime" size="medium" v-model="inPostPreProcess.human_touch_time" disabled style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
                <el-form-item label="开始处理时间">
                    <el-col :span="18">
                        <el-date-picker format type="datetime" size="medium" v-model="inPostPreProcess.start_time" disabled style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
                <el-form-item label="完成处理时间">
                    <el-col :span="18">
                        <el-date-picker format type="datetime" size="medium" v-model="inPostPreProcess.end_time" disabled style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
                <el-form-item label="备注">
                    <el-input type="textarea" :maxlength='255' placeholder="最多输入255个字符" v-model="inPostPreProcess.memo" :autosize="{ minRows: 2, maxRows: 4}"></el-input>
                </el-form-item>
        ...
        <el-dialog title="网格分配" :visible.sync="assignManualVisible" :close-on-click-modal="false">
            <el-form label-width="120px" class="demo-dynamic">
                <el-form-item label="网格列表">
                    <el-input type="textarea" readonly placeholder="请选择要需要分配的网格" :autosize="{ minRows: 5, maxRows: 5}" v-model="assignManualList.tileIDS"></el-input>
                </el-form-item>
                <p class="tipInfo-1">提示：已选择<b>{{selectNumber}}</b>条记录</p>
                <el-form-item label="作业员" required>
                    <el-cascader style="float:left;width:80%" :options="workerOptions" size="medium" filterable :change-on-select="WorkerChangeOnselect" class="left-div" placeholder="选择作业员" @visible-change="operatorVisible" @change="workerChange" :clearable="true" :show-all-levels="false" v-model="selectWorkerArr"></el-cascader>
                </el-form-item>
                <el-form-item>
