/**
 * @file汉字与拼音互转工具，根据导入的字典文件的不同支持不同
...
(function () {
    ...
    var pinyinUtil = {
        ...
        parseDict: function () {
            // 如果导入了 pinyinDictFirstletter.js
            if (window.pinyinDictFirstletter) {
                dict.firstletter = window.pinyinDictFirstletter;
            }
            // 如果导入了 pinyinDictNotone.js
            if (window.pinyinDictNotone) {
                dict.notone = {};
                dict.py2hz = window.pinyinDictNotone; // 拼音转汉字
                for (var i in window.pinyinDictNotone) {
                    var temp = window.pinyinDictNotone[i];
        ...
        getPinyin: function (chinese, splitter, withtone, polyphone) {
            ...
            if (dict.withtone) { // 优先使用带声调的字典文件
                ...
                if (withtone) {
                    console.warn('pinyinDictNotone 字典文件不支持声调！');
                }
                if (polyphone) {
                    console.warn('pinyinDictNotone 字典文件不支持多音字！');
                }
                var noChinese = '';
                for (var i = 0, len = chinese.length; i < len; i++) {
            ...
            } else if (window.pinyinDictPolyphone) {
    ...
    function parsePolyphone(chinese, result, splitter, withtone) {
        var poly = window.pinyinDictPolyphone;
